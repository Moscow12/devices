...
class SerialTcpApp:
    ...
    def _process_ecg_file(self, filepath: str):
        self.update_text(self.ecg_log_text,
                         f"[{_now()}] ✔ File received: {os.path.basename(filepath)}")

        # Parse the ECG file
        try:
            result = self.ecg_parser.parse(filepath)

            # Show patient info + summary
            summary  = f"File    : {result['filename']}\n"
            summary += f"Format  : {result['format']}\n"
            summary += f"Time    : {result['timestamp']}\n"
            patient  = result.get('patient', {})
            if patient:
                summary += "\n── Patient Info ──\n"
            ...
            if leads:
                summary += "\n── Lead Summary ──\n"
                for lead, samples in leads.items():
                    if samples:
                        mn = round(min(samples), 3)
                        mx = round(max(samples), 3)
                        summary += f"  {lead:5s}: {len(samples)} samples  min={mn} mV  max={mx} mV\n"
            summary += f"\n── Raw Header (hex) ──\n  {result['raw_hex']}"
            self.update_text(self.ecg_parsed_text, summary)
            ...
            # Forward to LIS API
            self._forward_to_lis(result, hl7)

        except Exception as e:
            self.update_text(self.ecg_log_text, f"[{_now()}] Parse error: {e}")

    def _forward_to_lis(self, result: dict, hl7: str):
        ...
        try:
            payload = {
                "source":    "edan_se1200",
                "device_no": result.get('patient', {}).get('DeviceID', '1871844'),
                "filename":  result['filename'],
                "timestamp": result['timestamp'],
                "patient":   result.get('patient', {}),
                "hl7":       hl7,
                "leads":     {
                    lead: samples[:100]   # send first 100 samples per lead
                    for lead, samples in result.get('leads', {}).items()
                    if samples
                },
            }
            resp = requests.post(api_url, json=payload,
                                 ...
                                 timeout=5)
            self.update_text(self.ecg_log_text,
                             f"[{_now()}] LIS API → {resp.status_code}: {resp.text[:80]}")
        except Exception as e:
            self.update_text(self.ecg_log_text,
                             f"[{_now()}] LIS API error: {e}")
